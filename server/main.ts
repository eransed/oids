...
class Client {
  ...
  updateNameOnce(newName: string) {
    if (!this.nameHasBeenUpdated) {
      ...
      console.error(
        `Multiple name updates for ${this.toString()}: newName="${newName}"`
      );
    }
  }
  ...
  addEventListeners(): void {
    ...
    this.ws.addEventListener("close", () => {
      ...
      console.log(
        `${this.toString()} has been disconnected, sending goodbye message`
      );
  ...
  toString(): string {
    return `${this.name} (${ipport(this.req)}, ${getReadyStateText(
      this.ws
    )}, added: ${this.dateAdded.toLocaleTimeString()})`;
  }
}
...
function addNewClientIfNotExisting(
  clients: Client[],
...
function removeClientIfExisting(
  clients: Client[],
  clientConnection: Client
): Client[] {
